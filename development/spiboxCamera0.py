#!/bin/bash
"""Setup components, take videos, and define their location and filename"""
...
try:
    ...
    def video(camera):
        """Take Video and append filename"""
        print('Motion detected! Taking snapshot')
        os.chdir(working_directories[1])
        camera.start_recording(get_file_name())
        camera.wait_recording(10)
        camera.stop_recording()
        os.chdir(working_directories[0])

    def my_callback_take_video(channel):
        """A Call back for a rising/falling edge event on PIR sensor to take a video"""
        if GPIO.input(pir_pin):
            print("Rising Edge detected on pin %s" % channel)
            video(initialized_camera)
        else:
    ...
    GPIO.add_event_detect(pir_pin, GPIO.BOTH, callback=my_callback_take_video)
